...
class StockExample(server.App):
    title = "Statistic Results (by Ira)"
    ...
    def region_first_week_SP ( data, params):
        list_1 = []

        for id in params['province']:
            k = id-1
            region_data = data[k]
            list_2 = region_data.VHI[(region_data['week'] == 1) & (region_data['VHI'] > 90)
            & (region_data['VHI'] > 0)].tolist()
            if len(list_2) > 0:
                list_1.append(id)
    ...
    def getPlot(self, params, data):
        df = self.getData(params)
        df = self.region_first_week_SP(data,params)
